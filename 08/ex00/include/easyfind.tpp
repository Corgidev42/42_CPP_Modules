#include <algorithm>
#include <stdexcept>

template<typename T>
typename T::iterator easyfind(T& container, int numberToFind){
  typename T::iterator it = std::find(container.begin(), container.end(), numberToFind);
  if (it == container.end())
    throw std::runtime_error("no matching value");
  return it;
}

template<typename T>
typename T::const_iterator easyfind(const T& container, int numberToFind){
  typename T::const_iterator it = std::find(container.begin(), container.end(), numberToFind);
  if (it == container.end())
    throw std::runtime_error("no matching value");
  return it;
}
